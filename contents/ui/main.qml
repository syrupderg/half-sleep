...
PlasmoidItem {
    ...
    function toggleScreen() {
        ...
        let blockArg = Plasmoid.configuration.blockSleep ? "--block-sleep" : "--no-block-sleep";
        let ptrArg = Plasmoid.configuration.disablePointers ? "--disable-pointers" : "--no-disable-pointers";
        let kbdBlockArg = Plasmoid.configuration.disableKeyboard ? "--disable-keyboard" : "--no-disable-keyboard";
        
        executable.connectSource("bash -c '~/.local/bin/toggle-screen.sh " + kbdArg + " " + volArg + " " + profArg + " " + blockArg + " " + ptrArg + " " + kbdBlockArg + "; echo " + Date.now() + "'")
    }
